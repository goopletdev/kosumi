...
class KosumiGoban {
    ...
    skipBackward() {
        ...
        this.boardState.innerText = this.prettify(this.activeNode.state);
    ...
    stepForeward() {
        if (this.activeNode.hasOwnProperty('children')) {
            ...
            this.boardState.innerText = this.prettify(newState);
        }
        this.info.value = `(Node: ${this.activeNode.id}) Move ${this.activeNode.moveNumber}:\n${JSON.stringify(this.activeNode.props)}`;
    }
...
//need to define getNodeById() and prettify(); and gameTree; get ride of getState() since this should only reference a fully-formed node tree
